...
pub struct ProverKey<F: PrimeField> {
    pub left_sigma: (Polynomial<F>, Evaluations<F>),
    pub right_sigma: (Polynomial<F>, Evaluations<F>),
    pub out_sigma: (Polynomial<F>, Evaluations<F>),
    pub fourth_sigma: (Polynomial<F>, Evaluations<F>),
    pub linear_evaluations: Evaluations<F>, // Evaluations of f(x) = X [XXX: Remove this and benchmark if it makes a considerable difference -- These are just the domain elements]
}
